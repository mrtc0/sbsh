...
// commandName is what a command may be called: a plain word, matched against the
// first word of a command line. There is no PATH to look a name up on, so a name
// that needs quoting to be typed could only ever confuse.
var commandName = regexp.MustCompile(`^[A-Za-z0-9][A-Za-z0-9_.-]*$`)

// WithCommand registers commands written in Go with the sandbox. A registered
// command is dispatched exactly like a builtin: it appears in pipelines, its
// streams can be redirected, and its exit status is the script's.
//
// Registration is per sandbox — there is no process-wide registry to add to — so
// two sandboxes in one program can offer different commands, and a test can
...
// plain word, has an empty description, or has a name a builtin or the shell
// itself already answers to. Each of those is a command that would silently
// never run, or would silently displace another, which is worth a construction
// error rather than a puzzle later.
...
func (s *Sandbox) addCommand(cmd command.Command) error {
	...
	case interp.IsBuiltin(name), syntax.IsKeyword(name):
		// A keyword is not a builtin, and the difference matters: a script
		// starting with "if" or "function" does not parse, and "time" measures
		// what follows it rather than running it. Either way the command could
		// never run.
...
// Commands returns the commands registered on the sandbox, sorted by name. It is
// what a host renders its own help or listing from; the sandbox has no help
// command of its own.
