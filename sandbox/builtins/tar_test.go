...
func Test_tar_createListExtract(t *testing.T) {
	...
	// Create.
	require.NoError(t, tarCommand(context.Background(), env, []string{"-cf", "out.tar", "src"}))
	_, err := env.FS.Stat("/work/out.tar")
	require.NoError(t, err)

	// List.
	require.NoError(t, tarCommand(context.Background(), env, []string{"-tf", "out.tar"}))
	list := stdout.String()
	...
	require.NoError(t, tarCommand(context.Background(), env2, []string{"-xf", "out.tar", "-C", "dest"}))
...
func Test_tar_createFromCurrentDir(t *testing.T) {
	...
	require.NoError(t, tarCommand(context.Background(), env, []string{"-cf", "out.tar", "."}))
	require.NoError(t, tarCommand(context.Background(), env, []string{"-tf", "out.tar"}))
...
func Test_tar_gzipRoundTrip(t *testing.T) {
	...
	require.NoError(t, tarCommand(context.Background(), env, []string{"-czf", "out.tgz", "f.txt"}))
	require.NoError(t, tarCommand(context.Background(), env, []string{"-xzf", "out.tgz", "-C", "restored"}))
...
func Test_tar_verbose(t *testing.T) {
	...
	require.NoError(t, tarCommand(context.Background(), env, []string{"-cvf", "out.tar", "only.txt"}))
...
func Test_tar_extractConfinesMembers(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			err := tarCommand(context.Background(), env, tt.args)
			if tt.wantErr {
				require.Error(t, err)
...
func Test_tar_extractKeepsMembersWrittenBeforeTheRejection(t *testing.T) {
	...
	require.Error(t, tarCommand(context.Background(), env, []string{"-xf", "out.tar", "-C", "dest"}))
...
func Test_tar_errors(t *testing.T) {
	...
	t.Run("no mode", func(t *testing.T) {
		t.Parallel()
		env, _, _ := NewTestEnv(t, "/work")
		require.Error(t, tarCommand(context.Background(), env, []string{"-f", "out.tar"}))
	})

	t.Run("create without files", func(t *testing.T) {
		t.Parallel()
		env, _, _ := NewTestEnv(t, "/work")
		require.Error(t, tarCommand(context.Background(), env, []string{"-cf", "out.tar"}))
	})

	t.Run("unknown flag", func(t *testing.T) {
		t.Parallel()
		env, _, _ := NewTestEnv(t, "/work")
		require.Error(t, tarCommand(context.Background(), env, []string{"-cq", "out.tar", "x"}))
	})
}
...
func Test_tar_createContinuesPastDeniedEntries(t *testing.T) {
	...
	err := tarCommand(context.Background(), env, []string{"-cf", "out.tar", "src"})
