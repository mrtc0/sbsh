...
func patchCommand(_ context.Context, inv *Invocation) error {
	fs := NewFlagSet()
...
func (a patchAction) apply(inv *Invocation) error {
	if a.remove {
...
func planPatches(inv *Invocation, patches []filePatch, strip int) ([]patchAction, error) {
...
func plannedLines(inv *Invocation, planned map[string]plannedFile, target patchTarget) ([]string, error) {
...
// never read.
func checkDeletable(inv *Invocation, planned map[string]plannedFile, target patchTarget) error {
	if p, ok := planned[target.abs]; ok {
		if p.gone {
...
func resolvePatchTarget(inv *Invocation, name string, strip int) (patchTarget, error) {
...
func writeLines(inv *Invocation, abs string, lines []string) error {
	content := ""
	if len(lines) > 0 {
