...
func (n *nester) only(t *testing.T) *command.ExecutionResult {
	t.Helper()
	n.mu.Lock()
	defer n.mu.Unlock()
	if len(n.errs) > 0 {
...
// TestNormalizeOutcome drives the shared normalization directly, which is the
// only way to cover what the shell backend does not produce today: reporting a
// stop as an exit status rather than as an error of its own. TimedOut and
// Canceled have to survive that, or a caller would be left inferring a stop from
// a status. Every execution goes through this, so what it decides is what both
// a host and a nested caller read.
func TestNormalizeOutcome(t *testing.T) {
	...
	for _, tc := range []struct {
		...
		},
		{
			...
			want:   command.ExecutionResult{ExitCode: 3},
		},
		{
			name:   "a deadline reported as its own error",
			ctx:    expired(),
			runErr: context.DeadlineExceeded,
			want:   command.ExecutionResult{TimedOut: true, ExitCode: exitcode.Timeout},
		},
		{
		...
		},
		{
			...
			want:   command.ExecutionResult{Canceled: true, ExitCode: exitcode.Canceled},
		},
		{
			name:   "anything else is an internal error",
			ctx:    context.Background(),
			runErr: errors.New("the runtime came apart"),
			want:   command.ExecutionResult{InternalError: "the runtime came apart", ExitCode: 1},
		},
	} {
		t.Run(tc.name, func(t *testing.T) {
			t.Parallel()

			var got command.ExecutionResult
			normalizeOutcome(tc.ctx, &got, tc.runErr)

			if got != tc.want {
				t.Errorf("normalizeOutcome() = %+v, want %+v", got, tc.want)
			}
		})
	}
}
