...
// TestSandbox_RegisteredCommand covers what a registered command has to be able
// to do to count as a command: take arguments, read the standard input it was
// given, reach the sandbox filesystem, read the environment, appear in a
// pipeline, and decide the script's exit status.
func TestSandbox_RegisteredCommand(t *testing.T) {
	t.Parallel()
	...
	cases := []struct {
		...
		{
			name:       "runs with its arguments",
			cmds:       []command.Command{echoArgs()},
			script:     "args a b c",
			wantStdout: "a|b|c\n",
		},
		{
			name:       "reads the script's standard input",
...
// TestSandbox_RegisteredCommandSeesTheDenyPatterns pins that a registered
// command is inside the sandbox rather than beside it: it is handed the same
// filesystem handle the builtins get, with the deny patterns already layered on.
func TestSandbox_RegisteredCommandSeesTheDenyPatterns(t *testing.T) {
	t.Parallel()
