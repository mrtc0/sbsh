...
| `WithCommand(cmds...)` | Register custom commands written in Go |
...
script's, and an unregistered name still prints `command not found`.
...
[examples/customcommand](examples/customcommand/main.go) is a complete program:
a `wordfreq` command reading files and standard input, registered and then used
in a script alongside the builtins. Dynamic loading and config-file
registration are not supported — a custom command is Go code compiled into the
host.
...
no-op. A host can add commands of its own in Go — see
