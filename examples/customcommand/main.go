...
// wordFreq is a custom command. The three methods are the whole interface.
...
// Run reads each argument as a file in the sandbox filesystem, or standard
// input when there is no argument or the argument is "-".
func (wordFreq) Run(_ context.Context, inv *command.Invocation) error {
	...
	for _, name := range sources {
		r := inv.Stdin
		if name != "-" {
			// inv.FS is the sandbox filesystem: mounts resolved, deny patterns
			// in force. inv.Abs resolves a relative argument against the
